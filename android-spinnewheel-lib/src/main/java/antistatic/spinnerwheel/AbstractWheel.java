...
public abstract class AbstractWheel extends View {
	...
	public AbstractWheel(Context context, AttributeSet attrs, int defStyle) {
		...
		initData(context);
		if (isInEditMode()) {
			final String[] itemList = new String[10];
			for (int i = 0; i < 10; ++i) {
				itemList[i] = String.format("Item %d", i);
			}
			final ArrayWheelAdapter<String> arrayWheelAdapter = new ArrayWheelAdapter<String>(context, itemList);
	...
	protected void initData(Context context) {
		...
		mScroller = createScroller(new WheelScroller.ScrollingListener() {
			...
			@Override
			public void onScroll(int distance, boolean notifyListeners) {
				doScroll(distance, notifyListeners);

				final int dimension = getBaseDimension();
			...
			@Override
			public void onJustify(boolean notifyListeners) {
				if (Math.abs(mScrollingOffset) > WheelScroller.MIN_DELTA_FOR_SCROLLING) {
					mScroller.scroll(mScrollingOffset, 0, notifyListeners);
				}
			}
		});
	}
	 ...
	 */
	public void scroll(int itemsToScroll, int time, boolean notifyListeners) {
		final int distance = itemsToScroll * getItemDimension() - mScrollingOffset;
		onScrollTouched(); // we have to emulate touch when scrolling spinnerwheel programmatically to light up stuff
		mScroller.scroll(distance, time, notifyListeners);
	}
	 ...
	 *            the scrolling value
	 */
	private void doScroll(int delta, boolean notifyListeners) {
		...
		final int offset = mScrollingOffset;
		if (pos != mCurrentItemIdx) {
			setCurrentItem(pos, false, notifyListeners);
		} else {
			invalidate();
		}

		// update offset
		final int baseDimension = getBaseDimension();
	...
	/**
	 * Sets the desired count of visible items. Actual amount of visible items depends on spinnerwheel layout parameters. To apply changes and rebuild view call measure().
	 *
	 * @param count
	 ...
	 * @param notifyListeners
	 */
	public void setCurrentItem(int index, boolean animated, boolean notifyListeners) {
		...
		if (index != mCurrentItemIdx) {
			if (animated && getItemDimension() > 0) {
				int itemsToScroll = index - mCurrentItemIdx;
				if (mIsCyclic) {
					final int scroll = itemCount + Math.min(index, mCurrentItemIdx) - Math.max(index, mCurrentItemIdx);
					if (scroll < Math.abs(itemsToScroll)) {
						itemsToScroll = itemsToScroll < 0 ? scroll : -scroll;
					}
				}
				scroll(itemsToScroll, 0, notifyListeners);
			} else {
				mScrollingOffset = 0;
	...
	protected boolean rebuildItems() {
		...
		if (mItemsLayout != null) {
			final int first = mRecycler.recycleItems(mItemsLayout, mFirstItemIdx, range);
	...
	private View getItemView(int index) {
		...
		final int count = mViewAdapter.getItemsCount();
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			if (!mIsScrollingPerformed) {
				...
				final int items = distance / getItemDimension();
